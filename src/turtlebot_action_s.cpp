...
turtlebot_action_s::turtlebot_action_s(ros::NodeHandle nh, std::string name) : node(nh), as(nh, name, false),
                                                                               turtleName(name) {
    ...
    subBringUpCallback = node.subscribe("/" + turtleName + "/mobile_base/events/robot_state", 10,
                                        &turtlebot_action_s::waitKobukiCallback, this);
    ...
    for (auto it = robotNames.begin(); it != robotNames.end(); it++) //initialize list of clients
    {
        ...
        } else  // retrieve parameters for this server
        {
            ...
            if (node.getParam("turtleList/xPIDThet/" + currentName, tempXPIDThetGains)) {
                xPidController.setGains(tempXPIDThetGains[0], tempXPIDThetGains[1], tempXPIDThetGains[2],
                                        tempXPIDThetGains[3]);
            ...
            if (node.getParam("turtleList/yPIDThet/" + currentName, tempYPIDThetGains)) {
                yPidController.setGains(tempYPIDThetGains[0], tempYPIDThetGains[1], tempYPIDThetGains[2],
                                        tempYPIDThetGains[3]);
    ...
    while (initFlag) {
        ...
        if (turtleClients[indexNextServer]->isClientConnected() and
            (launchMode == "gazebo" or (launchMode == "turtlebot" and initKobuki == true))) {
...
void turtlebot_action_s::executeCallback(const rosgraph_msgs::Clock &msg) {
...
// advertise the kokuby node to apply the velocity
        ...
        srand(turtleID + int(ros::Time::now().toNSec()));
